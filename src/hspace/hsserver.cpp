...
BOOL CHSServer::ProcessInput(int des)
{
	...
	if (nread > 0)
	{
		...
		if (aidx != -1 && m_sockdata[aidx])
		{
			...
			// it.
			sprintf(cmdbuf, "%s%s", m_sockdata[aidx],
				tbuf);
	...
	switch(cmd)
	{
		case SC3_NUMBER_OF:
			...
			sprintf(retcmd, "%d%d%d%d%d",
				SC1_OBJECT, SC3_INFO, SC3_NUMBER_OF, type,
				nObjs);
...
void CHSServer::ProcessClassCommand(char *strCmd, int des)
{
	...
	switch(cmd)
	{
		...
		case SC3_DELETE:
			...
			{
				sprintf(retcmd,
					"%dFailed to delete specified class.",
					SC1_MESSAGE);
				SendCommand(retcmd, des);
				return;
			}
	...
	switch(cmd)
	{
		case SC3_DETAILS:
			...
			for (idx = 0; idx < HS_MAX_CLASSES; idx++)
			{
				...
				if (sclass &&
					!strcmp(sclass->m_name, classname))
					break;
			}
			if (!sclass)
