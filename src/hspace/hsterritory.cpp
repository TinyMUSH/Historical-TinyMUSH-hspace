...
// specified type.
CHSTerritory *CHSTerritoryArray::NewTerritory(int objnum,
											  TERRTYPE type)
{
		...
		if (m_territories[idx])
		{
			switch(m_territories[idx]->GetType())
			{
				case T_RADIAL:
					...
					notify(player, 
						tprintf("[%5d] %-18s RADIAL  %-3d Center: %d,%d,%d  Radius: %d",
						m_territories[idx]->GetDbref(),
						Name(m_territories[idx]->GetDbref()),
						m_territories[idx]->GetUID(),
						cRadial->GetX(),
						cRadial->GetY(),
						cRadial->GetZ(),
						cRadial->GetRadius()));
				...
				case T_CUBIC:
					...
					notify(player, 
						tprintf("[%5d] %-18s CUBIC   %-3d Min: %d,%d,%d  Max: %d,%d,%d",
						m_territories[idx]->GetDbref(),
						Name(m_territories[idx]->GetDbref()),
						m_territories[idx]->GetUID(),
						cCubic->GetMinX(),
...
CHSTerritory *CHSTerritoryArray::InTerritory(CHSObject *cObj)
{
	...
	for (int idx = 0; idx < HS_MAX_TERRITORIES; idx++)
	{
		if (m_territories[idx])
		{
			if (m_territories[idx]->PtInTerritory(
				uid, x, y, z))
...
CHSTerritory *CHSTerritoryArray::FindTerritory(int objnum)
{
	...
	{
		if (m_territories[idx] && 
			m_territories[idx]->GetDbref() == objnum)
			return m_territories[idx];
	}
	// No match
	return NULL;
