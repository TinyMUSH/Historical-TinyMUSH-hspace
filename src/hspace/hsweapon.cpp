...
void CHSWeaponDBArray::WriteLaser(CHSDBLaser *weapon, FILE *fp)
{
	fprintf(fp, "0\"%s\" %d %d %d %d %d %d\n",
		...
		weapon->m_range, weapon->m_accuracy, weapon->m_powerusage, weapon->m_nohull);
}
...
void CHSWeaponArray::NewLaser(int wclass)
{
	...
	// Set the parent
	m_weapons[m_nWeapons]->SetParent(
		waWeapons.GetWeapon(wclass, HSW_LASER));
}
...
void CHSWeaponArray::NewMTube(int wclass)
{
	...
	m_weapons[m_nWeapons]->SetParent(
		waWeapons.GetWeapon(wclass, HSW_MISSILE));
}
...
BOOL CHSLaser::CanAttackObject(CHSObject *cObj)
{
	...
	if (cObj->GetType() == HST_SHIP || cObj->GetType() == HST_MISSILE && !NoHull())
	{
		return TRUE;
	}
	return FALSE;
