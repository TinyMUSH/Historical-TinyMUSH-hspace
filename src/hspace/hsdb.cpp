...
BOOL CHSDB::LoadObjectDB(void)
{
  ...
  while(fgets(tbuf, 256, fp))
  {
	  ...
	  switch(key)
	  {
			case HSK_NOKEY:
				sprintf(tbuf, 
					"WARNING: Invalid key \"%s\" encountered.",
					strKey);
				hs_log(tbuf);
				break;
...
CHSCelestial *CHSDB::FindCelestial(int objnum)
{
	...
	for (idx = 0; idx < HS_MAX_UNIVERSES; idx++)
	{
		...
		// types.
		if ((rCel= (CHSCelestial *)unDest->FindObject(objnum, HST_PLANET)))
			return rCel;
	}
...
void CHSDB::CreateNewUniverse(dbref player, char *strRoom)
{
	...
	if (!uDest)
	{
		notify(player, "Unable to create new universe.  Check log for errors.");
		return;
	}
	...
	notify(player, tprintf("Universe %s (#%d) created.  Behold!",
...
void CHSDB::DeleteUniverse(dbref player, char *strID)
{
	...
	{
	  notify(player, tprintf("No such universe with id %d.", uid));
	  return;
	}
