...
void CHSFuelSystem::GiveSystemInfo(int player)
{
	...
	// Now our information
	notify(player,
		tprintf("BURNABLE FUEL: %.2f", m_burnable_fuel));
	notify(player,
		tprintf("REACTABLE FUEL: %.2f", m_reactable_fuel));
	notify(player,
		tprintf("MAX REACTABLE FUEL: %d", GetMaxFuel(FL_REACTABLE)));
	notify(player,
		tprintf("MAX BURNABLE FUEL: %d",GetMaxFuel(FL_BURNABLE)));
}

BOOL CHSFuelSystem::SetAttributeValue(char *strName,
									  char *strValue)
{
	int iVal;
