...
HSPACE_FUNC_HDR(hsfSpaceMsg)
{
	...
	if (hsInterface.HasFlag(executor, TYPE_THING,
		THING_HSPACE_OBJECT))
	...
	switch(how)
	{
		...
		case 2:
			...
			for (i = 0; i < HS_MAX_ACTIVE_OBJECTS; i++)
			{
				...
				cSensors = (CHSSysSensors *)cTarget->GetEngSystem(HSS_SENSORS);
				if (!cSensors)
					continue;
				...
				if (cContact->status == DETECTED &&
					how == 1)
					cTarget->HandleMessage(args[1], 
					MSG_SENSOR, (long *)cObj);
				else if (cContact->status == IDENTIFIED &&
					how == 2)
					cTarget->HandleMessage(args[1],
					MSG_SENSOR, (long *)cObj);
			}
...
HSPACE_FUNC_HDR(hsfGetEngSystems)
{
	...
	if (hsInterface.HasFlag(dbObj, TYPE_THING,
		THING_HSPACE_OBJECT))
		cObj = dbHSDB.FindObject(dbObj);
	else if (hsInterface.HasFlag(dbObj, TYPE_THING,
		THING_HSPACE_CONSOLE))
		cObj = dbHSDB.FindObjectByConsole(dbObj);
	else
		cObj = NULL;
		...
		if (!*rbuf) {
			if (!cShip)
				sprintf(tbuf, "%d:#%d:%d:%d:%s:%d:%d:%.4f",
					...
					cTarget->GetType(),
					cTarget->GetSize(),
					cContact->status == DETECTED ? "Unknown" : cTarget->GetName(),
					XYAngle(sX, sY, tX, tY),
					ZAngle(sX, sY, sZ, tX, tY, tZ),
					Dist3D(sX, sY, sZ, tX, tY, tZ));
			else
...
HSPACE_FUNC_HDR(hsfSysAttr)
{
	...
	*ptr = '\0';
	...
	if (type == HSS_NOTYPE && cObj->GetType() == HST_SHIP || type == HSS_FICTIONAL && cObj->GetType() == HST_SHIP)
	{
		CHSShip *dObj;
		dObj = (CHSShip *)cObj;
...
HSPACE_FUNC_HDR(hsfDelSys)
{
	...
	if (cObj->GetType() != HST_SHIP)
		return "#-1 That HSpace object does not currently support that operation.";
	...
	if(pShip->m_systems.DelSystem(cSys))
		return tprintf("%s system deleted from the %s.", tmp, pShip->GetName());
	else
...
HSPACE_FUNC_HDR(hsfAddSys)
{
	...
	return tprintf("%s system added to the %s.", cSys->GetName(), pShip->GetName());
}
