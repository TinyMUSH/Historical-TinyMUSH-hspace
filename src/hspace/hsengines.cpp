...
void CHSSysEngines::GiveSystemInfo(int player)
{
	...
	CHSEngSystem::GiveSystemInfo(player);
 	...
 	// Now our info
	notify(player,
		tprintf("MAX VELOCITY: %d", GetMaxVelocity(FALSE)));
	notify(player,
		tprintf("ACCELERATION: %d", GetAcceleration(FALSE)));
	notify(player,
		tprintf("DESIRED SPEED: %.0f", m_desired_speed));
	notify(player,
		tprintf("CURRENT SPEED: %.0f", m_current_speed));
	notify(player,
		tprintf("CAN AFTERBURN: %s", CanBurn() ? "YES" : "NO"));
	notify(player,
		tprintf("AFTERBURNING : %s", GetAfterburning() ? "YES" : "NO"));
	notify(player,
...
BOOL CHSSysEngines::SetDesiredSpeed(int speed)
{
	...
	if ((speed > iMaxVelocity) ||
		   (speed < (-1 * (iMaxVelocity / 2.0))))
	{
		return FALSE;
	}
...
void CHSSysEngines::ConsumeFuelBySpeed(int speed)
{
	...
	if (!m_fuel_source || 
		(m_fuel_source->GetFuelRemaining() <= 0))
		return;
...
void CHSJumpDrive::GiveSystemInfo(int player)
{
	...
	CHSEngSystem::GiveSystemInfo(player);
 	...
 	// Now our info
	notify(player,
		tprintf("CHARGE: %.2f", m_charge_level));
	notify(player,
		tprintf("ENGAGED: %s", m_engaged ? "YES" : "NO"));
	notify(player,
		tprintf("MIN SPEED: %d", GetMinJumpSpeed()));
	notify(player,
		tprintf("EFFICIENCY: %d", GetEfficiency()));
}
		...
		if (rate > 0)
		{
			...
			if (m_charge_level == 100) {
				if (m_ownerObj && m_ownerObj->GetType() == HST_SHIP) {
					...
					cShip = (CHSShip *)m_ownerObj;
					if (cShip)
						cShip->NotifyConsoles(tprintf("%s%s-%s Jump Drive charged.", ANSI_HILITE, ANSI_GREEN, ANSI_NORMAL), MSG_ENGINEERING);
			} }
