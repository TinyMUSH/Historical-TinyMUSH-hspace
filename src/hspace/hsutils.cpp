...
void hs_log(char *str)
{
  ...
  if (text) {
	...
	STARTLOG(LOG_ALWAYS, "HSPACE", "INFO")
		log_text(newstr);
	ENDLOG
  }
  free(newstr);
  ...
  sprintf(tbuf, "%s: %s", timebuf, str);
  fprintf(spacelog_fp, "%s\n", tbuf);
  fflush(spacelog_fp);
#endif
}
...
// are needed for this operation.
int XYAngle(double fx, double fy, 
			double sx, double sy)
{
  char    tbuf[8];
  int     ang;
...
void hsStdError(int player, char *strMsg)
{
	...
	sprintf(strNewMsg,
		"%s%s-%s %s",
		ANSI_HILITE, ANSI_GREEN, ANSI_NORMAL, strMsg);
