...
void CHSShip::SetVelocity(dbref player, int iVel)
{
	...
	{
	  hsStdError(player,
		  "Unable to set speed during surface exchange.");
	  return;
	}
	...
	{
		hsStdError(player,
			"Unable to set speed while docking.");
		return;
	}
	...
	if ((iVel > iMaxVelocity) ||
		   (iVel < (-1 * (iMaxVelocity / 2.0))))
	{
		sprintf(tbuf, 
	...
	{
		sprintf(tbuf, 
			"Desired velocity now set to %d hph.", iVel);
		hsStdError(player, tbuf);
	}
	else
	{
		hsStdError(player,
		"Failed to set desired velocity.");
		return;
	}
...
void CHSShip::SetHeading(dbref player, int iXYAngle, int iZAngle)
{
	...
	{
		hsStdError(player,
			"Unable to change course while docking.");
		return;
	}
	...
	{
		hsStdError(player,
			"This vessel has no maneuvering thrusters.");
		return;
	}
	...
	{
		hsStdError(player,
			"Thrusters are not currently online.");
		return;
	}
	...
	{
		notify(player,
			"Valid XY headings range from 0 - 359.");
		return;
	}
	...
	{
		notify(player,
			"Valid Z headings range from -90 - 90.");
		return;
	}
...
void CHSShip::SetRoll(dbref player, int iRoll)
{
	...
	{
		hsStdError(player,
			"Valid rolls reach from 0 to 359 degrees.");
		return;
	}

	m_desired_roll = iRoll;
	hsStdError(player, 
		tprintf("Ship roll now set to %i degrees.", iRoll));
...
#ifdef PENNMUSH
			  did_it(m_objnum, newterr, NULL, NULL, NULL, NULL,
				  "AENTER", NOTHING);
#endif
#ifdef TM3
...
#ifdef PENNMUSH
			did_it(m_objnum, oldterr, NULL, NULL, NULL, NULL,
				  "ALEAVE", NOTHING);
#endif
#ifdef TM3
...
void CHSShip::RecomputeVectors(void)
{
	...
	CHSVector tvec(
		d2sin_table[xyhead] * d2cos_table[zhead],
	...
	for (idx = 3; mLine1[idx]; idx++)
    {
		if (mLine1[idx] != ' ')
		{
			...
			if (ptr)
				sprintf(tbuf3, 
				"%s%c%s", 
				ptr, mLine1[idx], ANSI_NORMAL);
			else
				sprintf(tbuf3, 
				"%s%c%s", 
				ANSI_HILITE, mLine1[idx], ANSI_NORMAL);
		}
		else
			sprintf(tbuf3, " ");
		strcat(tbuf2, tbuf3);
    ...
    for (idx = 1; mLine2[idx]; idx++)
    {
		if (mLine2[idx] != ' ')
		{
			...
			if (ptr)
				sprintf(tnum1, 
				"%s%c%s", 
				ptr, mLine2[idx], ANSI_NORMAL);
			else
				sprintf(tnum1, 
				"%s%c%s", 
				ANSI_HILITE, mLine2[idx], ANSI_NORMAL);
		}
		else
			sprintf(tnum1, " ");
		strcat(tbuf3, tnum1);
    ...
    for (idx = 0; mLine3[idx]; idx++)
    {
		if (mLine3[idx] != ' ')
		{
			...
			if (ptr)
				sprintf(tnum1, 
				"%s%c%s", 
				ptr, mLine3[idx], ANSI_NORMAL);
			else
				sprintf(tnum1, 
				"%s%c%s", 
				ANSI_HILITE, mLine3[idx], ANSI_NORMAL);
		}
		else
			sprintf(tnum1, " ");
      strcat(tbuf3, tnum1);
    ...
    for (idx = 0; mLine4[idx]; idx++)
    {
		if (mLine4[idx] != ' ')
		{
			...
			if (ptr)
				sprintf(tnum1, 
				"%s%c%s", 
				ptr, mLine4[idx], ANSI_NORMAL);
			else
				sprintf(tnum1, 
				"%s%c%s", 
				ANSI_HILITE, mLine4[idx], ANSI_NORMAL);
		}
		else
			sprintf(tnum1, " ");
		strcat(tbuf3, tnum1);
    ...
    for (idx = 0; mLine5[idx]; idx++)
    {
		...
		else if (mLine5[idx] != ' ')
		{
			...
			if (ptr)
				sprintf(tnum1, 
				"%s%c%s", 
				ptr, mLine5[idx], ANSI_NORMAL);
			else
				sprintf(tnum1, 
				"%s%c%s", 
				ANSI_HILITE, mLine5[idx], ANSI_NORMAL);
		}
		else
			sprintf(tnum1, " ");
		strcat(tbuf3, tnum1);
    ...
    for (idx = 0; mLine6[idx]; idx++)
    {
		if (mLine6[idx] != ' ')
		{
			...
			if (ptr)
				sprintf(tnum1, 
				"%s%c%s", 
				ptr, mLine6[idx], ANSI_NORMAL);
			else
				sprintf(tnum1, 
				"%s%c%s", 
				ANSI_HILITE, mLine6[idx], ANSI_NORMAL);
		}
		else
			sprintf(tnum1, " ");
		strcat(tbuf3, tnum1);
    ...
    for (idx = 0; mLine7[idx]; idx++)
    {
		if (mLine7[idx] != ' ')
		{
			...
			if (ptr)
				sprintf(tnum1, 
				"%s%c%s", 
				ptr, mLine7[idx], ANSI_NORMAL);
			else
				sprintf(tnum1, 
				"%s%c%s", 
				ANSI_HILITE, mLine7[idx], ANSI_NORMAL);
		}
		else
			sprintf(tnum1, " ");
		strcat(tbuf3, tnum1);
    ...
    for (idx = 1; mLine8[idx]; idx++)
    {
		if (mLine8[idx] != ' ')
		{
			...
			if (ptr)
				sprintf(tnum1, 
				"%s%c%s", 
				ptr, mLine8[idx], ANSI_NORMAL);
			else
				sprintf(tnum1, 
				"%s%c%s", 
				ANSI_HILITE, mLine8[idx], ANSI_NORMAL);
		}
		else
			sprintf(tnum1, " ");
		strcat(tbuf3, tnum1);
...
// celestial surface.
void CHSShip::LandVessel(dbref player, int id, 
						 int loc, char *lpstrCode)
{
	SENSOR_CONTACT *cContact;
	CHSSysSensors  *cSensors;
	CHSSysEngines  *cEngines;
	...
	{
		hsStdError(player,
			"You may not do that during surface exchange.");
		return;
	}
	...
	{
		hsStdError(player,
			"This vessel has no engines.  Unable to land.");
		return;
	}
	...
	{
		hsStdError(player,
			"Invalid landing code -- permission denied.");
		return;
	}
	...
	if (m_dock_status > 0)
	{
		...
		switch(m_dock_status)
		{
			case 6:		// Bay doors open
				...
				MSG_GENERAL);
				m_landing_target->HandleMessage(
...
void CHSShip::UndockVessel(dbref player)
{
	...
	if (m_dock_status)
	{
		if (m_dock_status < 0)
			hsStdError(player,
				HSCONF.ship_is_undocking);
		else
			hsStdError(player,
				HSCONF.ship_is_docking);
		return;
	}
	if (m_drop_status)
	{
	...
	{
		hsStdError(player,
			"You may not undock from this location.");
		return;
	}
	...
	if (m_dock_status < 0)
	{
		...
		switch(m_dock_status)
		{
			...
			case -2:		// Bay doors open
				...
				GetName());
				hsInterface.NotifyContents(m_landing_target->objnum,
					tbuf);
				break;
			...
			case 0:
				...
				{
					NotifyConsoles(
						"Unable to find source ship.  Putting you where you docked at.", MSG_GENERAL);
					uDest = uaUniverses.FindUniverse(m_uid);
				}
				...
				{
					NotifyConsoles(
						"Help!  I can't find your universe!",
						MSG_GENERAL);
					return;
				}
	...
	else if (m_drop_status < 0)
	{
		...
		else if (m_drop_status == iTestLifters)
		{
			...
			sprintf(tbuf,
				...
				GetName());
			hsInterface.NotifyContents(
				m_landing_target->objnum, tbuf);
		}
...
void CHSShip::EngageAfterburn(dbref player, BOOL bStat)
{
	...
	{
		hsStdError(player,
			"Unable to engage afterburners at this time.");
		return;
	}
	...
	{
		hsStdError(player,
			"Our engines are not capable of afterburning.");
		return;
	}
	...
	{
		hsStdError(player,
			"Engines are not currently online.");
		return;
	}
	...
	{
		hsStdError(player,
			"Afterburners already engaged.");
		return;
	}
	else if (!bStat && !cEngines->GetAfterburning())
	{
		hsStdError(player,
			"The afterburners are not currently engaged.");
		return;
	}
	...
	else
	{
		...
		if (!cEngines->SetAfterburn(FALSE))
			hsStdError(player,
			"Failed to disengage afterburners.");
		else
		{
			sprintf(tbuf,
...
void CHSShip::EngageJumpDrive(dbref player, BOOL bStat)
{
	...
	{
		hsStdError(player,
			"Unable to engage jump drive at this time.");
		return;
	}
	...
	{
		hsStdError(player,
			"Jump drive is not currently online.");
		return;
	}
	...
	{
		hsStdError(player,
			"Jump drive already engaged.");
		return;
	}
	else if (!bStat && !cJumpers->GetEngaged())
	{
		hsStdError(player,
			"The jump drive is not currently engaged.");
		return;
	}
	...
	if (bStat)
	{
		...
		{
			hsStdError(player,
				"This vessel doesn't even have engines.");
			return;
		}
	...
	else
	{
		...
		if (!cJumpers->SetEngaged(FALSE))
			hsStdError(player,
			"Failed to disengage jump drive");
		else
		{
			ExitHyperspace();
		}
...
void CHSShip::DoBreakBoardLink(dbref player, int slot)
{
	...
	{
		hsStdError(player,
			"No hatch with that notation found.");
		return;
	}
	int port;
	...
	{
		hsStdError(player,
			"Local hatch is clamped can't break link.");
		return;
	}
	...
	{
		hsStdError(player,
			"Remote hatch is clamped can't break link.");
		return;
	}
	...
	cHatch->HandleMessage(tprintf("%s is disconnected.", Name(cHatch->objnum)), MSG_GENERAL);
	lHatch->HandleMessage(tprintf("%s is disconnected.", Name(lHatch->objnum)), MSG_GENERAL);
...
void CHSShip::DoBoardLink(dbref player, int id, int lhatch, int dhatch)
{
	...
	sprintf(tbuf, "The %s has engaged docking couplings on our hatch %d.", GetName(), lhatch);
	cShip->NotifyConsoles(tbuf, MSG_GENERAL);
...
void CHSShip::ViewObjectID(dbref player, int id)
{
	...
	if (cContact->status == IDENTIFIED)
		if (hsInterface.AtrGet(cContact->m_obj->GetDbref(),"DESCRIBE"))
			notify(player, tprintf("%s",hsInterface.EvalExpression(hsInterface.m_buffer,cContact->m_obj->GetDbref(),m_objnum,m_objnum)));
		else
			notify(player, "No description set on that object.");
	else {
		hsStdError(player,
			"Contact outside visual range.");
		return; }
}
...
void CHSShip::GateObjectID(dbref player, int id)
{
	...
	if (Dist3D(GetX(), GetY(), GetZ(), cObj->GetX(), cObj->GetY(), cObj->GetZ()) > 2) {
	...
	if (cObj->GetType() == HST_WORMHOLE) {
		...
		if (cWorm->GetSize() / 2 < GetSize()) {
			hsStdError(player, "This ship is too large to gate the wormhole.");
			return; }
...
void CHSShip::EngageCloak(dbref player, BOOL bStat)
{
	...
	{
		hsStdError(player,
			"This vessel has no cloaking device.");
		return;
	}
	...
	{
		hsStdError(player,
			"Cloaking Device already engaged.");
		return;
	}
...
void CHSShip::GiveHatchRep(dbref player)
{
	...
	sprintf(tbuf, 
		...
		ANSI_HILITE, ANSI_BLUE, ANSI_GREEN, ANSI_NORMAL, ANSI_HILITE, ANSI_GREEN, ANSI_NORMAL,
		ANSI_HILITE, ANSI_GREEN, ANSI_NORMAL, ANSI_HILITE, ANSI_GREEN, ANSI_BLUE, ANSI_NORMAL);
	notify(player, tbuf);
	...
	for (idx = 1; idx < hatches + 1; idx++)
	{
		...
		sprintf(tbuf, "%s%s|%s[%s%i%s%s]%s  %-36s  %11s    %s%s|%s",
			ANSI_HILITE, ANSI_BLUE, ANSI_GREEN, ANSI_NORMAL, idx, ANSI_HILITE, 
			ANSI_GREEN, ANSI_NORMAL, tbuf2, tbuf3, 
			ANSI_HILITE, ANSI_BLUE, ANSI_NORMAL);
		notify(player, tbuf);
	}
