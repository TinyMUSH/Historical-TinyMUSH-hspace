...
void CHSShip::GiveEngSysReport(dbref player)
{
	...
	for (cSys = m_systems.GetHead(); cSys; cSys = cSys->GetNext())
	{
		...
		*strStress = '\0';
		...
		sprintf(strPower, "%3d (%d)",
			cSys->GetCurrentPower(),
			cSys->GetOptimalPower());
	...
	if (cComputer)
	{
		...
		notify(player, tbuf);
		char tbuf2[128];
		sprintf(tbuf2, "%d/%d", cComputer->GetUsedPower(), cComputer->GetCurrentPower());
		sprintf(tbuf, "%s%s|%s Power Usage: %-25s Surplus: %-18s %s%s|%s",
			ANSI_HILITE, ANSI_BLUE, ANSI_NORMAL, tbuf2, tprintf("%d",cComputer->GetPowerSurplus()),
			ANSI_HILITE, ANSI_BLUE, ANSI_NORMAL);
		notify(player, tbuf);
		sprintf(tbuf2, "%d", cComputer->GetPoweredConsoles());
		sprintf(tbuf, "%s%s|%s Consoles   : %-25s Powered: %-18s %s%s|%s",
			ANSI_HILITE, ANSI_BLUE, ANSI_NORMAL, tprintf("%d", cComputer->GetConsoles()),
			tbuf2 , ANSI_HILITE,
	...
	if (cReactor)
	{
		...
		*strStress = '\0';
		...
		sprintf(tbuf,
			...
			ANSI_HILITE, ANSI_BLUE, ANSI_NORMAL,
			strStress, strDamage, ANSI_HILITE, ANSI_BLUE,
			ANSI_NORMAL);
		notify(player, tbuf);
	}
				...
				if (idx < fLvl)
				{
					if (idx < 3)
					{
						sprintf(strTmp, "%s%s%c%s", 
							ANSI_HILITE, ANSI_BRED,
							c, ANSI_NORMAL);
					}
					else if (idx < 7)
					{
						sprintf(strTmp, "%s%s%c%s", 
							ANSI_HILITE, ANSI_BYELLOW,
							c, ANSI_NORMAL);
					}
					else
					{
						sprintf(strTmp, "%s%s%c%s", 
							ANSI_HILITE, ANSI_BGREEN,
							c, ANSI_NORMAL);
					}
				}
				else
							  ...
							  BOOL bPercent)
{
	...
	else if (cSys->IsVisible()) // Has to be visible
	{
		...
		{
			hsStdError(player, "Insufficient power available for that operation.");
			return;
		}
...
// on the ship.
CHSSysShield *CHSShip::DetermineHitShield(int iXYAngle, 
										  int iZAngle)
{
	CHSSysShield *cFore;
	CHSSysShield *cAft;
...
void CHSShip::GiveCrewRep(dbref player)
{
	...
	{
		hsStdError(player,
			"This vessel has no damage control systems.");
		return;
	}
	...
	sprintf(tbuf, 
		...
		ANSI_HILITE, ANSI_GREEN, ANSI_NORMAL, ANSI_HILITE, ANSI_GREEN, ANSI_NORMAL, ANSI_HILITE,
		ANSI_GREEN, ANSI_NORMAL, ANSI_HILITE, ANSI_GREEN, ANSI_BLUE, ANSI_NORMAL);
	notify(player, tbuf);
	...
	for (idx = 1; idx < cDamCon->GetNumCrews() + 1; idx++)
	{
		...
		sprintf(tbuf, "%s%s|%s[%s%i%s%s]%s  %-20s %-15s %10s %10s     %s%s|%s",
			ANSI_HILITE, ANSI_BLUE, ANSI_GREEN, ANSI_NORMAL, idx, ANSI_HILITE, 
			ANSI_GREEN, ANSI_NORMAL, tbuf2, cDamCon->GetCyclesLeft(idx) ? tprintf("%i", 
			cDamCon->GetCyclesLeft(idx)) : "     Idle      ", strDamage, strDamage2,
			ANSI_HILITE, ANSI_BLUE, ANSI_NORMAL);
		notify(player, tbuf);
	}
