...
BOOL CHSShip::AddHatch(dbref dbExit)
{
	...
	hsInterface.AtrAdd(dbExit, "HSDB_SHIP",
		tprintf("#%d", m_objnum), GOD, AF_MDARK || AF_WIZARD);
...
void CHSShip::HandleSystems(void)
{
	...
	if (m_self_destruct_timer > 0)
	{
		...
		{
			NotifySrooms(tprintf("%s%s*** SELF DESTRUCT IN PROGRESS ***%s",
			...
			if (m_self_destruct_timer == 3600) {
				NotifySrooms(tprintf("%s%s*** SELF DESTRUCT IN 1 HOUR ***%s",
					ANSI_RED, ANSI_HILITE, ANSI_NORMAL));
			} else if (m_self_destruct_timer == 1200) {
				NotifySrooms(tprintf("%s%s*** SELF DESTRUCT IN 20 MINUTES ***%s",
					ANSI_RED, ANSI_HILITE, ANSI_NORMAL));
			} else if (m_self_destruct_timer == 300) {
				NotifySrooms(tprintf("%s%s*** SELF DESTRUCT IN 5 MINUTES ***%s",
					ANSI_RED, ANSI_HILITE, ANSI_NORMAL));
			} else if (m_self_destruct_timer == 120) {
				NotifySrooms(tprintf("%s%s*** SELF DESTRUCT IN 2 MINUTES ***%s",
					ANSI_RED, ANSI_HILITE, ANSI_NORMAL));
			} else if (m_self_destruct_timer == 60) {
				NotifySrooms(tprintf("%s%s*** SELF DESTRUCT IN 1 MINUTE ***%s",
					ANSI_RED, ANSI_HILITE, ANSI_NORMAL));
			} else if (m_self_destruct_timer == 30) {
				NotifySrooms(tprintf("%s%s*** SELF DESTRUCT IN 30 SECONDS ***%s",
					ANSI_RED, ANSI_HILITE, ANSI_NORMAL));
			} else if (m_self_destruct_timer == 20) {
				NotifySrooms(tprintf("%s%s*** SELF DESTRUCT IN 20 SECONDS ***%s",
					ANSI_RED, ANSI_HILITE, ANSI_NORMAL));
			} else if (m_self_destruct_timer < 11) {
				NotifySrooms(tprintf("%s%s*** SELF DESTRUCT IN %d SECONDS ***%s",
	...
	for (idx = 0; idx < MAX_HATCHES; idx++)
	{
		...
		if (bBreak)
		{
			...
			if (cHatch)
			{
				...
				Location(lHatch->objnum) = NOTHING;
			...
			NotifyConsoles(tprintf("Docking couplings on hatch %d disengaged.", idx + 1), MSG_GENERAL);
		}
	}
}
...
void CHSShip::DisembarkPlayer(dbref player, int id)
{
	...
	hsInterface.NotifyContents(dbLoc,
		tprintf("%s disembarks from the %s.",
		Name(player), GetName()));
						   ...
						   int iSysType)
{
	...
	else
	{
		...
		if (m_hull_points < 0)
		{
			...
			// Trigger akill
#ifdef PENNMUSH
			did_it(dbAttacker, m_objnum, NULL, NULL, NULL, NULL,
				"AKILL", NOTHING);
#endif
#ifdef TM3
						 ...
						 CHSShip *newShip)
{
	...
	// rooms.
#ifdef PENNMUSH
	dbref dbNewRoom = new_object();

	// Set some info on the new room.
	set_name(dbNewRoom, Name(room));
	Owner(dbNewRoom) = GOD;
	Flags(dbNewRoom) = TYPE_ROOM;
	Flags(dbNewRoom) |= options.room_flags;
	Toggles(dbNewRoom) |= options.room_toggles;
	Toggles(dbNewRoom) |= ROOM_FLOATING;
    Owner(dbNewRoom) = Owner(room);
    db[dbNewRoom].parent = db[room].parent;

	// Copy room attributes
	atr_cpy(dbNewRoom, room);
#endif
#ifdef TM3
		...
		exit_m = Next(exit_m))
	{
			...
			{
#ifdef PENNMUSH
				// Open an exit to the new room.
				dbNewExit = new_object();

				/*
				 * initialize everything
				 */
				set_name(dbNewExit, Name(exit_m));
				Owner(dbNewExit) = Owner(exit_m);
				Zone(dbNewExit) = Zone(exit_m);
				Exits(dbNewExit) = dbNewRoom;
				Flags(dbNewExit) = TYPE_EXIT;
				Parent(dbNewExit) = Parent(exit_m);
				Flags(dbNewExit) |= options.exit_flags;
				Toggles(dbNewExit) |= options.exit_toggles;

				// Copy attributes
				atr_cpy(dbNewExit, exit_m);
#endif
#ifdef TM3
				...
				// Link it in
#ifdef PENNMUSH
				PUSH(dbNewExit, Exits(dbNewRoom));
#endif
			}
			...
			if (rRoom != NOTHING)
			{
					...
					exit_opposite = Next(exit_opposite))
				{
					...
					{
#ifdef PENNMUSH
						// Found a return exit to model!
						dbNewExit = new_object();

						/*
						 * initialize everything
						 */
						set_name(dbNewExit, Name(exit_opposite));
						Owner(dbNewExit) = Owner(exit_opposite);
						Zone(dbNewExit) = Zone(exit_opposite);
						Exits(dbNewExit) = rRoom;
						Flags(dbNewExit) = TYPE_EXIT;
						Flags(dbNewExit) |= options.exit_flags;
						Toggles(dbNewExit) |= options.exit_toggles;

						// Copy attributes
						atr_cpy(dbNewExit, exit_opposite);

						/*
						 * link it in
						 */
						PUSH(dbNewExit, Exits(rRoom));

						Location(dbNewExit) = dbNewRoom;
#endif
#ifdef TM3
...
void CHSShip::InitSelfDestruct(dbref player, int Timer, char *BoardCode)
{
	if (GetBoardingCode())
	{
		if(strcmp(BoardCode, m_boarding_code)) {
			notify(player, tprintf("%s%s-%s Invalid access code.",
				ANSI_HILITE, ANSI_GREEN, ANSI_NORMAL)); 
			return;	
		}
	}
	...
	if(Timer < 1)
	{
		if(m_self_destruct_timer > 0)
		{
			NotifySrooms(tprintf("%s%sSelf destruct aborted.%s",
				ANSI_HILITE, ANSI_GREEN, ANSI_NORMAL));
			NotifyConsoles(tprintf("%s%s-%s Self destruct aborted.",
				ANSI_HILITE, ANSI_GREEN, ANSI_NORMAL), MSG_GENERAL);
			m_self_destruct_timer = 0;
		} else {
			notify(player, tprintf("%s%s-%s Self destruct not in progress.",
				ANSI_GREEN, ANSI_HILITE, ANSI_NORMAL));
		}
	} else {
		NotifySrooms(tprintf("%s%s*** SELF DESTRUCT IN %d SECONDS ***%s",
			ANSI_HILITE, ANSI_RED, Timer, ANSI_NORMAL));
		NotifyConsoles(tprintf("%s%s-%s Self destruct initiated.",
...
{
	hsInterface.AtrAdd(objnum, "HSDB_TARGETOBJ", tprintf("#%d", m_targetObj), GOD, AF_MDARK || AF_WIZARD);
	hsInterface.AtrAdd(objnum, "HSDB_TARGETHATCH", tprintf("%d", m_targetHatch), GOD, AF_MDARK || AF_WIZARD);
	hsInterface.AtrAdd(objnum, "HSDB_CLAMPED", tprintf("%d", m_clamped), GOD, AF_MDARK || AF_WIZARD);
}
