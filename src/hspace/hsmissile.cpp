...
void CHSMissile::ChangeHeading(void)
{
	...
	if (bChange)
	{
		...
		CHSVector tvec(
			d2sin_table[m_xyheading] * d2cos_table[zang],
...
void CHSMissile::HitTarget(void)
{
	...
	m_target->HandleDamage(m_source_object, 
			(CHSWeapon *)this,
			dbMiss->m_strength, m_source_console, HSS_NOTYPE);
}
                              ...
                              int iSysType)
{
	...
	hsStdError(player,
		"Your shot explodes the target missile!");
