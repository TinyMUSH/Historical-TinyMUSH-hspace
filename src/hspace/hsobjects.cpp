...
void CHSObject::HandleMessage(char *lpstrMsg,
							  int msgType,
							  long *data)
{
	// Do nothing.
...
BOOL CHSObject::LoadFromFile(FILE *fp)
{
	...
	// Set type
	hsInterface.AtrAdd(m_objnum, "HSDB_TYPE", tprintf("%d",
		m_type), GOD);
	...
	else
	{
		if (!uDest->AddObject(this))
		{
			sprintf(tbuf, "WARNING: HSpace object #%d was not be added to universe.",
				m_objnum);
			hs_log(tbuf);
		}
	}
...
void CHSObject::GiveScanReport(CHSObject *cScanner, 
							   dbref player,
							   BOOL id)
{
	char tbuf[256];
