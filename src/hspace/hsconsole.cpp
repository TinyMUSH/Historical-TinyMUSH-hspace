...
void CHSConsole::WriteObjectAttrs(void)
{
	...
	for (idx = 0; idx < HS_MAX_MESSAGE_TYPES; idx++)
	{
		...
		hsInterface.AtrAdd(m_objnum, tbuf,
			tprintf("%d", m_msgtypes[idx]), GOD, AF_MDARK || AF_WIZARD);
		...
		*tbuf = '\0';
		for (idx = 0; idx < HS_MAX_WEAPONS; idx++)
		{
			if (cWeap = m_weapon_array->GetWeapon(idx))
			{
				...
				hsInterface.AtrAdd(m_objnum, strWeaponID,
					tbuf2, GOD);
			}
		}
...
void CHSConsole::LockTargetID(dbref player, int ID)
{
	...
	{
		hsStdError(player,
			"Target is out of our max weapons range.");
		return;
	}
	...
	hsStdError(player,
		"Weapons now locked on specified target contact.");
}
...
BOOL CHSConsole::AddWeapon(dbref player, int type)
{
	...
	if (!m_weapon_array->NewWeapon(type))
		...
		notify(player,
		tprintf("Weapon type %d added to console.", type));
...
void CHSConsole::ConfigureWeapon(dbref player,
								 int weapon,
								 int type)
{
	CHSWeapon *cWeap;
	...
	{
		hsStdError(player,
			"That weapon type cannot be configured.");
		return;
	}
	...
	{
		hsStdError(player,
			"No munitions storage available.");
		return;
	}
	...
	{
		hsStdError(player,
			tprintf("Invalid munitions ID (%d) specified.",
			type+1));
		return;
	}
	...
	// All went ok.
	hsStdError(player,
		"Weapon configured to specified munitions type.");
}
...
void CHSConsole::LoadWeapon(dbref player, int weapon)
{
	...
	{
		hsStdError(player,
			"That weapon is already currently loading.");
		return;
	}
	...
	{
		hsStdError(player,
			"Unable to reload that weapon at this time.");
		return;
	}
				...
				if (cWeap = m_weapon_array->GetWeapon(idx))
				{
					if (cWeap->GetStatusChange() == STAT_READY)
					{
						if (cWeap->Loadable())
							...
							ANSI_HILITE, ANSI_GREEN, ANSI_NORMAL,
							ANSI_HILITE, idx+1, ANSI_GREEN, ANSI_NORMAL);
						else
...
void CHSConsole::UnlockWeapons(dbref player)
{
	...
	// Tell the player
	hsStdError(player,
		"Weapons now unlocked from previous target.");
}
...
void CHSConsole::ChangeHeading(dbref player, int iXY, int iZ)
{
	...
	char tbuf[128];
	sprintf(tbuf, "Console heading changed to %d mark %d.",
		iXY, iZ);
...
void CHSConsole::FireWeapon(dbref player, int weapon)
{
	...
	{
		hsStdError(player,
			"That weapon is not ready for firing.");
		return;
	}
	...
	{
		hsStdError(player,
			"Target is not currently within firing cone.");
		return;
	}
...
void CHSConsole::PowerUp(dbref player)
{
	...
	{
		hsStdError(player,
			"This console is already online.");
		return;
	}
...
void CHSConsole::PowerDown(dbref player)
{
	...
	{
		hsStdError(player,
			"This console is not currently online.");
		return;
	}
	...
	if (m_computer)
	{
		...
		m_online = FALSE;
		hsStdError(player,
			tprintf("%s powered down.", Name(m_objnum)));
	}
	else
		hsStdError(player,
		"That's not necessary with this console.");
}
	...
	if (iVertLine == 2)
	{
			...
			iVertLine > 1 && iVertLine < 7)
		{
			...
			sprintf(tbuf,
				...
				ANSI_BLUE, ANSI_RED,
				tbuf2,
				ANSI_BLUE, ANSI_GREEN, ANSI_BLUE,
				ANSI_NORMAL);
		}
		else
		{
...
void CHSConsole::UnloadWeapon(dbref player, int weapon)
{
	...
	{
		hsStdError(player,
			"That weapon cannot be unloaded.");
		return;
	}

	if (!cWeap->Unload())
		hsStdError(player,
		"Failed to unload specified weapon.");
	else
		hsStdError(player,
		"Weapon unloaded.");
}
...
void CHSConsole::SetAutoload(dbref player, BOOL bStat)
{
	if (!bStat)
	{
		m_autoload = FALSE;
		hsStdError(player,
			"Autoloading deactivated.");
	}
	else
	{
		m_autoload = TRUE;
		hsStdError(player,
			"Autoloading activated.");
	}
}
...
void CHSConsole::SetSystemTarget(dbref player, int type)
{
	...
	else
	{
		...
		if (!ptr)
			...
			hsStdError(player,
			tprintf("Weapons now targeting %s system on enemy target.",
			ptr));
	}
}
