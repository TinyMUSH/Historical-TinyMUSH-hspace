...
HSPACE_COMMAND_HDR(hscSetVelocity)
{
	...
	{
		notify(dbUser,
			"This vessel has been destroyed!");
		return;
	}
	...
	{
		hsStdError(dbUser,
			tprintf("%s is currently powered down.",
			Name(player)));
		return;
	}
...
HSPACE_COMMAND_HDR(hscLandVessel)
{
	...
	{
		notify(dbUser, 
			"You must specify a target ID/location.");
		return;
	}
...
HSPACE_COMMAND_HDR(hscUndockVessel)
{
	...
	if (cShip->GetMannedConsoles() < cShip->GetMinManned()) {
		hsStdError(dbUser, tprintf("Only %d out of a minimum of %d consoles are manned.", cShip->GetMannedConsoles(), cShip->GetMinManned()));
		return; }
...
HSPACE_COMMAND_HDR(hscDoBoardLink)
{
	...
	*dptr = '\0';
	...
	{
		notify(dbUser, 
			"You must specify a target hatch.");
		return;
	}
	...
	{
		notify(dbUser, 
			"You must specify a target contact.");
		return;
	}
	if (!right_switch || !*right_switch)
	{
		notify(dbUser, 
			"You must specify a local hatch.");
		return;
	}
	// Tell the ship to board link.
	cShip->DoBoardLink(dbUser, atoi(left_switch), atoi(right_switch), atoi(arg_right));
}
...
HSPACE_COMMAND_HDR(hscTaxiVessel)
{
	...
	{
		notify(dbUser, 
			"You must specify a direction to taxi.");
		return;
	}
	...
	{
		hsStdError(dbUser,
			"Must be landed or docked to taxi.");
		return;
	}
	...
	{
		notify(dbUser,
			"That is not a valid direction to taxi.");
		return;
	}
	...
	{
		hsStdError(dbUser,
			"That location is blocked to our departure.");
		return;
	}
	moveto(dbObj, Location(dbExit));

	hsInterface.NotifyContents(Location(dbObj),
	   tprintf("The %s fires is vector rockets and taxis to %s.", 
	   cShip->GetName(), Name(Location(dbExit))));
	moveto(dbObj, Location(dbExit));
	hsInterface.NotifyContents(Location(dbExit),
		 tprintf("The %s taxis in, firing its vector rockets.", 
		 cShip->GetName()));
	hsStdError(dbUser, 
		"You fire the vector rockets and taxi onward.");
...
HSPACE_COMMAND_HDR(hscTractorMode)
{
	...
	{
		hsStdError(dbUser,
			"No tractor beam system on this vessel.");
		return;
	}
	...
	{
		hsStdError(dbUser,
			"Invalid tractor beam mode setting.");
		return;
	}
