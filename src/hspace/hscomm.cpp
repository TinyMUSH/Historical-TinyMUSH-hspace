...
void CHSCommRelay::RelayCommlinks(HSCOMM *commdata)
{
	...
	for (idx = 0; idx < hsInterface.MaxObjects(); idx++)
	{
		if (!hsInterface.HasFlag(idx, TYPE_THING,
			THING_HSPACE_COMM) || 
			hsInterface.HasFlag(idx, TYPE_THING,
		...
		numwenv = 3;
#endif
		hsInterface.EvalExpression(hsInterface.m_buffer, idx,
			idx, idx);
	}
}
...
BOOL CHSCommRelay::RelayMessage(HSCOMM *commdata)
{
	...
	for (idx = 0; idx < HS_MAX_OBJECTS; idx++)
	{
		...
		if (dDist < cJammer->GetRange(TRUE))
		{
			...
			if(tShip)
				tShip->NotifyConsoles(tprintf("%s%s-%s You are being jammed, source bearing %d mark %d.",
					ANSI_HILITE, ANSI_GREEN, ANSI_NORMAL,
					XYAngle(dX, dY, commdata->sX, commdata->sY),
					ZAngle(dX, dY, dZ, commdata->sX, commdata->sY, commdata->sZ)), MSG_GENERAL);
