...
void CHSEngSystem::DoCycle(void)
{
	...
	if ((m_current_power > iOptPower) ||
		(m_stress > 0))
	{
		// Calculate percent overload/underload
		fVal = (m_current_power - iOptPower) / 
				(float) iOptPower;
		fVal *= 100;
...
{
	notify(player,
		tprintf("Name     : %s", GetName()));
	notify(player,
		tprintf("Visible  : %s", m_bVisible ? "YES" : "NO"));
	notify(player,
		tprintf("Damage   : %d", m_damage_level));
	notify(player,
		tprintf("Tolerance: %d", GetTolerance())); 
	notify(player,
		tprintf("Opt Power: %d", GetOptimalPower(FALSE))); 
	notify(player,
		tprintf("Cur Power: %d", m_current_power));
	notify(player,
		tprintf("Stress   : %.1f", m_stress));
}
...
void CHSSysSensors::GiveSystemInfo(int player)
{
	...
	notify(player,
		tprintf("SENSOR RATING: %d", GetSensorRating()));
}
...
void CHSSysSensors::DoCycle(void)
{
	...
	for (idx = 0; iDetectCap && (idx < HS_MAX_ACTIVE_OBJECTS); idx++)
	{
		...
		dDetectDist = cObj->GetSize() / 2.0 * fCloakEffect * fNebulaEffect *
						(HSCONF.detectdist * 2.0) * GetSensorRating();
		dIdentDist  = cObj->GetSize() / 2.0 * fCloakEffect * fNebulaEffect *
...
void CHSSysSensors::NewContact(CHSObject *cObj, CONTACT_STATUS stat)
{
	...
	{
		hs_log("WARNING: Sensor array has reached maximum number of contacts.");
		delete cNewContact;
	}
...
void CHSReactor::GiveSystemInfo(int player)
{
	...
	notify(player,
		tprintf("MAX OUTPUT: %d", GetMaximumOutput(FALSE)));
	notify(player,
		tprintf("DESIRED OUTPUT: %d", m_desired_output));
	notify(player,
		tprintf("CURRENT OUTPUT: %d", m_output));
	notify(player,
		tprintf("EFFICIENCY: %d", GetEfficiency()));
}
		...
		if (fNeeded > 0)
		{
			...
			if (fRealized < fNeeded)
			{
				...
				sprintf(tbuf,
					...
					ANSI_HILITE, ANSI_YELLOW, ANSI_NORMAL);
				if (m_ownerObj)
					m_ownerObj->HandleMessage(
					tbuf, MSG_ENGINEERING, NULL);
...
void CHSSysThrusters::GiveSystemInfo(int player)
{
	...
	notify(player,
		tprintf("TURNING RATE: %d", GetRate(FALSE)));
}
...
UINT CHSSysShield::DoDamage(int iPts)
{
	...
	for (idx = 0; (idx < HS_MAX_ACTIVE_OBJECTS); idx++)
	{
		...
		dDistance = Dist3D(NebX, NebY, NebZ, m_ownerObj->GetX(), m_ownerObj->GetY(), m_ownerObj->GetZ());
...
void CHSSysShield::GiveSystemInfo(int player)
{
	...
	notify(player,
		tprintf("REGEN RATE: %.2f", GetRegenRate(FALSE)));
	notify(player,
		tprintf("STRENGTH: %.0f", m_strength));
	notify(player,
		tprintf("MAX STRENGTH: %.0f", GetMaxStrength()));
	notify(player,		tprintf("SHIELD TYPE: %s (%d)", 
		m_shield_type == ST_DEFLECTOR ? "Deflector" : "Absorption",
		m_shield_type));
}
...
void CHSSysComm::GiveSystemInfo(int player)
{
	...
	notify(player,
		tprintf("MAX RANGE: %d", GetMaxRange(FALSE)));
}
...
{
	if(bEngage) {
		m_engaging = atoi(tprintf("%.0f", GetOptimalPower() / 10.00));
		if(!IsEngaging())
			m_engaged = 1;
	} else {
		m_engaged = 0;
...
void CHSDamCon::DoCycle(void)
{
	...
	if (GetCurrentPower() != 0 && GetOptimalPower(FALSE) != 0)
		pinf = 100 * ((GetCurrentPower() * 1.00) / (GetOptimalPower(FALSE) * 1.00));
	else
		pinf = 0;
		...
		if(GetCyclesLeft(idx) <= 0) {
			...
			if (tSys->GetDamageLevel() == DMG_NONE) {
				...
				if(m_ownerObj->GetType() == HST_SHIP) {
					...
					tShip->NotifyConsoles(tprintf("%s%s-%s %s repairs complete.",
						ANSI_HILITE, ANSI_GREEN, ANSI_NORMAL, tSys->GetName()), MSG_ENGINEERING);
				}
			} else {
				int cycleswork = 500;
				int numworking = 0;
				...
				if(m_ownerObj->GetType() == HST_SHIP) {
					...
					tShip = (CHSShip *)m_ownerObj;
					tShip->NotifyConsoles(tprintf("%s%s-%s %s repaired continueing to repair to full.",
						ANSI_HILITE, ANSI_GREEN, ANSI_NORMAL, tSys->GetName()), MSG_ENGINEERING);
			} }
...
void CHSDamCon::AssignCrew(dbref player, int iCrew, CHSEngSystem *tSys)
{
	...
	if (tSys) {
		...
		if(m_ownerObj->GetType() == HST_SHIP) {
			...
			tShip = (CHSShip *)m_ownerObj;
			tShip->NotifyConsoles(tprintf("%s%s-%s Damage Control crew %i assigned to %s.",
			ANSI_HILITE, ANSI_GREEN, ANSI_NORMAL, iCrew, tSys->GetName()), MSG_ENGINEERING);
		} 
	} else {
		CHSEngSystem *cSys;
		...
		if(m_ownerObj->GetType() == HST_SHIP) {
			...
			tShip = (CHSShip *)m_ownerObj;
			tShip->NotifyConsoles(tprintf("%s%s-%s Damage Control crew %i now idle.",
			ANSI_HILITE, ANSI_GREEN, ANSI_NORMAL, iCrew), MSG_ENGINEERING);
		} 
	}
}
...
char *CHSSysTractor::GetAttrValueString(void)
{
	...
	sprintf(rval,
		...
		CHSEngSystem::GetAttrValueString(),	m_mode, GetStrength(FALSE));
...
void CHSSysTractor::DoCycle(void)
{
	...
	cDist = Dist3D(tObj->GetX(), tObj->GetY(), tObj->GetZ(), cShip->GetX(), cShip->GetY(), cShip->GetZ());

	if (cDist > (GetEffect() * 10)) {
		m_lock = NULL;
		cShip->NotifyConsoles(tprintf("%s%s-%s Tractor lock target out of range, releasing lock.",
			ANSI_GREEN, ANSI_HILITE, ANSI_NORMAL), MSG_GENERAL);
		tObj->HandleMessage("Tractor lock has been released.", MSG_GENERAL, (long *)cShip);
		return; }

	if (!cShip->IsActive() || cShip->IsDestroyed() || !tObj->IsActive() ||
		cShip->GetDockedLocation() != NOTHING) {
		m_lock = NULL;
		cShip->NotifyConsoles(tprintf("%s%s-%s Tractor lock lost.",
			ANSI_GREEN, ANSI_HILITE, ANSI_NORMAL), MSG_GENERAL);
		tObj->HandleMessage("Tractor lock has been released.", MSG_GENERAL, (long *)cShip);
		return; }
	...
	if (m_mode == HSTM_TRACTOR) {
		if (cDist > GetEffect()) {
			tObj->MoveInDirection(
				XYAngle(tObj->GetX(), tObj->GetY(), cShip->GetX(), cShip->GetY()),
				ZAngle(tObj->GetX(), tObj->GetY(), tObj->GetZ(), cShip->GetX(), cShip->GetY(), cShip->GetZ()),
				GetEffect());
		} else {
		...
		tObj->MoveInDirection(
			XYAngle(cShip->GetX(), cShip->GetY(), tObj->GetX(), tObj->GetY()),
			ZAngle(cShip->GetX(), cShip->GetY(), cShip->GetZ(), tObj->GetX(), tObj->GetY(), tObj->GetZ()),
			GetEffect());
		...
		tObj->MoveInDirection(cShip->GetXYHeading(), cShip->GetZHeading(), mvAmt);
	}
	cDist = Dist3D(tObj->GetX(), tObj->GetY(), tObj->GetZ(), cShip->GetX(), cShip->GetY(), cShip->GetZ());
...
void CHSSysTractor::SetMode(int mode)
{
	...
	if (cShip)
	{
		...
		if (mode == 0)
			...
			sprintf(tbuf, "hold");
		cShip->NotifyConsoles(tprintf("%s%s-%s Tractor beam mode set to %s.",
			ANSI_HILITE, ANSI_GREEN, ANSI_NORMAL, tbuf), MSG_GENERAL);
	}
...
void CHSSysTractor::SetLock(dbref player, int id)
{
	...
	{
		if (m_lock) {
			cShip->NotifyConsoles(tprintf("%s%s-%s Tractor beam unlocked.",
				ANSI_GREEN, ANSI_HILITE, ANSI_NORMAL), MSG_GENERAL);
			m_lock = NULL;
			return;
		} else {
	...
	{
		hsStdError(player,
			"This vessel is not equipped with sensors.");
		return;
	}
	...
	if (Dist3D(cShip->GetX(), cShip->GetY(), cShip->GetZ(), cObj->GetX(), cObj->GetY(), cObj->GetZ()) > GetStrength(TRUE) / cObj->GetSize() * 10.00) {
	...
	cShip->HandleMessage("Tractor beam locked onto target.", MSG_GENERAL, (long *)cObj);
	cObj->HandleMessage(tprintf("Tractor beam lock detected from the %s.", cShip->GetName()), MSG_GENERAL, (long *)cShip);
	if (cObj->GetType() == HST_SHIP) {
		CHSShip *tShip;
		tShip = (CHSShip *)cObj;
		tShip->NotifySrooms("The ship shakes lightly as something locks onto the hull."); }
}
...
void CHSSysTractor::DockShip(dbref player, int id, int loc)
{
	...
	if (Dist3D(cShip->GetX(), cShip->GetY(), cShip->GetZ(), cObj->GetX(), cObj->GetY(), cObj->GetZ()) > 2) {
		hsStdError(player, "Target outside of tractor docking range(2 hm).");
		return; }
		...
		{
			hsStdError(player,
				"We are too small to accomodate that vessel.");
			return;
		}
		...
		{
			hsStdError(player, 
				"That landing location does not exist.");
			return;
		}
		...
		{
 				hsStdError(player,
 					"The bay doors to that bay are closed.");
 				return;
 		}
		...
		{
			hsStdError(player,
				"Landing site cannot accomodate this vessel.");
			return;
		}
		...
		cShip->HandleMessage(tprintf("The %s is being tractored into docking bay %d.",
