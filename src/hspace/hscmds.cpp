...
HSPACE_COMMAND *hsFindCommand(char *switches, HSPACE_COMMAND *cmdlist)
{
	HSPACE_COMMAND *ptr;
	int len;
	...
	for (ptr = cmdlist; ptr->key; ptr++)
	{
		...
		if (!strncmp(switches, ptr->key, len))
			return ptr;
	}
...
HSPACE_COMMAND_HDR(hscCheckSpace)
{
	...
	notify(player, tprintf("   The system is currently %s.",
	...
	// Cycle time
	notify(player, tprintf("   Time per cycle: %.1fms", 
		HSpace.CycleTime()));
	...
	notify(player, tprintf("   Weapons    : %4d", dbHSDB.NumWeapons()));
	notify(player, tprintf("   Classes    : %4d", caClasses.NumClasses()));
	notify(player, tprintf("   Universes  : %4d", dbHSDB.NumUniverses()));
	notify(player, tprintf("   Ships      : %4d (%d)",uShips,uActiveShips));
	notify(player, tprintf("   Celestials : %4d", uCelestials));
	notify(player, tprintf("   Territories: %4d", taTerritories.NumTerritories()));
}
...
HSPACE_COMMAND_HDR(hscActivateShip)
{
	...
	{
		notify(player, tprintf("%s(#%d) is not of type THING.",
	...
	newShip->TotalRepair();
	notify(player, tprintf("%s (#%d) - activated with class %d.",
...
HSPACE_COMMAND_HDR(hscMothballObject)
{
	...
	notify(player, tprintf("HSpace object(#%d) mothballed.", dbObj));
}
...
HSPACE_COMMAND_HDR(hscAddLandingLoc)
{
	...
	if (!cObj)
	{
		notify(player, tprintf("That thing(#%d) is not an HSpace object.",
			dbConsole));
		return;
	}
	...
	switch(cObj->GetType())
	{
		case HST_SHIP:
			...
			if (!cShip->AddLandingLoc(dbRoom))
				...
				notify(player,
				"Landing location added to ship.");
			break;
		...
		case HST_PLANET:
			...
			if (!cPlanet->AddLandingLoc(dbRoom))
				...
				notify(player,
				"Landing location added to planet.");
			break;
	}
}
...
HSPACE_COMMAND_HDR(hscSetAttribute)
{
	...
	*dptr = '\0';
	...
	// Check flags first.
	if (hsInterface.HasFlag(dbConsole, TYPE_ROOM,
		ROOM_HSPACE_LANDINGLOC))
	{
		...
		if (!llLoc)
		{
			notify(player, "That thing appears to be a landing location, but it's not.");
			return;
		}
		...
		THING_HSPACE_CONSOLE))
	{
		...
		if (!cConsole)
		{
			notify(player, "That console does not belong to any HSpace object.");
			return;
		}
		...
		if (!cConsole->SetAttributeValue(right_switch, arg_right))
		{
			if (!cObj->SetAttributeValue(right_switch, arg_right))
				...
				notify(player, 
				tprintf("%s Attribute - cleared.", cObj->GetName()));
			else
				notify(player, 
				tprintf("%s Attribute - set.", cObj->GetName()));
		}
		else
		...
		THING_HSPACE_TERRITORY))
	{
		...
		notify(player, "That thing does not appear to be an HSpace object.");
}
...
HSPACE_COMMAND_HDR(hscManConsole)
{
	...
	if (!hsInterface.HasFlag(dbConsole, TYPE_THING,
		THING_HSPACE_CONSOLE))
	{
	...
	if (hsInterface.ConsoleUser(dbConsole) != dbConsole && hsInterface.AtrGet(hsInterface.ConsoleUser(dbConsole), "MCONSOLE")) {
		hsInterface.AtrDel(hsInterface.ConsoleUser(dbConsole), "MCONSOLE", GOD); }

#ifdef ONLINE_REG
	// Unregistered players can't use the system.
	if (hsInterface.HasFlag(player, TYPE_PLAYER,
		PLAYER_UNREG))
	{
		notify(player,
	...
	if(hsInterface.AtrGet(player, "MCONSOLE")) 
	{
		...
		notify(player, tprintf("You unman the %s.", Name(dbOldConsole)));
#ifdef PENNMUSH
		notify_except(db[Location(dbOldConsole)].contents, player,
			tprintf("%s unmans the %s.", Name(player), Name(dbOldConsole)));
#endif
#ifdef TM3
	...
	hsInterface.AtrAdd(player, "MCONSOLE", tprintf("#%d",dbConsole), GOD, AF_MDARK || AF_WIZARD);

	// Give some messages
	notify(player, tprintf("You man the %s.", Name(dbConsole)));

#ifdef PENNMUSH
	notify_except(db[Location(dbConsole)].contents, player,
	  tprintf("%s mans the %s.", Name(player), Name(dbConsole)));
#endif
#ifdef TM3
...
HSPACE_COMMAND_HDR(hscUnmanConsole)
{
	...
	if (!hsInterface.HasFlag(dbConsole, 
		TYPE_THING, THING_HSPACE_CONSOLE))
	{
	...
	notify(player, tprintf("You unman the %s.", Name(dbConsole)));
#ifdef PENNMUSH
	notify_except(db[Location(dbConsole)].contents, player,
		tprintf("%s unmans the %s.", Name(player), Name(dbConsole)));
#endif
#ifdef TM3
	notify_except(Location(dbConsole), player, player,
		tprintf("%s unmans the %s.", Name(player), Name(dbConsole)));
...
HSPACE_COMMAND_HDR(hscAddObject)
{
	...
	notify(player, tprintf("%s (#%d) - is now a new %s in universe 0.",
...
HSPACE_COMMAND_HDR(hscSetSystemAttr)
{
	...
	*dptr = '\0';
	...
	{
		if ((*sptr == '/') ||
			((dptr - sysname) > 63)) // Don't allow overflow
		{
			break;
		}
		else
	...
	*dptr = '\0';
	...
	if (hsInterface.HasFlag(dbObj, TYPE_THING,
		THING_HSPACE_CONSOLE))
...
HSPACE_COMMAND_HDR(hscDisembark)
{
	...
	{
		notify(player,
			"You cannot disembark from here.");
		return;
	}
...
HSPACE_COMMAND_HDR(hscBoardShip)
{
	...
	if (!hsInterface.HasFlag(dbShipObj, 
		TYPE_THING, THING_HSPACE_OBJECT))
	{
	...
	hsInterface.NotifyContents(dbPrevLoc,
		tprintf("%s boards the %s.",
		Name(player), Name(dbShipObj)));

	// Set the player's HSPACE_LOCATION attr.
        hsInterface.AtrAdd(player, "HSPACE_LOCATION",
                tprintf("#%d", dbShipObj), GOD, AF_MDARK | AF_WIZARD);
}
...
HSPACE_COMMAND_HDR(hscSetMissile)
{
	...
	*dptr = '\0';
		...
		THING_HSPACE_CONSOLE))
	{
		...
		{
			notify(player, 
				"You can only do that for ships.");
			return;
		}
		cShip = (CHSShip *) cObj;
	}
	...
	cShip->SetNumMissiles(player, atoi(strType),
		atoi(arg_right));
}
...
HSPACE_COMMAND_HDR(hscNewClass)
{
	...
	{
		notify(player,
			"Must specify a name for the class.");
		return;
	}
	...
	{
		notify(player,
			tprintf("Name length may not exceed %d characters.",
...
HSPACE_COMMAND_HDR(hscDumpClassInfo)
{
	...
	{
		notify(player,
			"Ship class non-existent.");
		return;
	}

	// Print out the info
	notify(player,
		tprintf("Ship Class: %d   %s",
		iClass, pClass->m_name));
	notify(player,
		"------------------------------------------------------------");
	notify(player,
		tprintf("Ship Size : %-3d         Cargo Size: %d",
		pClass->m_size, pClass->m_cargo_size));
	notify(player,
		tprintf("Min. Crew : %-3d         Max Hull  : %d",
...
HSPACE_COMMAND_HDR(hscAddSysClass)
{
	...
	{
		notify(player,
			"Must specify a system name to add.");
		return;
	}
	...
	notify(player, 
		tprintf("%s system added to class %d.",
		cSys->GetName(), iClass));
}
...
HSPACE_COMMAND_HDR(hscSetAttrClass)
{
	...
	*dptr = '\0';
	...
	if (!strncasecmp(attrname, "NAME", len))
	{
		...
		{
			notify(player,
				tprintf("Class name length may not exceed %d characters.",
				SHIP_CLASS_NAME_LEN - 1));
			return;
		}
...
HSPACE_COMMAND_HDR(hscCloneObject)
{
	...
	{
		notify(player,
			"You may only clone ships at this time.");
		return;
	}
	...
	if (dbShipObj == NOTHING)
		...
		notify(player, tprintf("Clone ship - cloned #%d", dbShipObj));
}
...
HSPACE_COMMAND_HDR(hscAddTerritory)
{
	...
	{
		notify(player,
			"Must specify type of territory to add.");
		return;
	}
	...
	if (!territory)
		...
		notify(player,
		tprintf("%s (#%d) added as a new territory.",
		Name(dbObj), dbObj));
}
...
HSPACE_COMMAND_HDR(hscDelTerritory)
{
	...
	{
		notify(player,
			"No such territory with that dbref.");
		return;
	}

	if (!taTerritories.RemoveTerritory(dbObj))
		notify(player,
		"Delete territory - failed.");
	else
		notify(player,
		"Delete territory - deleted.");
}
...
HSPACE_COMMAND_HDR(hscAddSys)
{
	...
	notify(player, 
		tprintf("%s system added to the %s.",
...
HSPACE_COMMAND_HDR(hscDelSys)
{
	...
	if(pShip->m_systems.DelSystem(cSys))
		notify(player, 
			tprintf("%s system deleted from the %s.",
			tmp, pShip->GetName()));
	else
...
HSPACE_COMMAND_HDR(hscDelSysClass)
{
	...
	{
		notify(player,
			"Must specify a system name to delete.");
		return;
	}
	...
	if(pClass->m_systems.DelSystem(cSys))
		notify(player, 
			tprintf("%s system deleted from class %d.",
			tmp, iClass));
	else
		notify(player, "Failed to delete system.");
}
...
HSPACE_COMMAND_HDR(hscDumpWeapon)
{
 	...
 	notify(player, tprintf("Weapon: %d    %s", iWeapon,
 		pWeapon->m_name));
 	notify(player,
		"------------------------------------------------------------");
 
 	if(pWeapon->m_type == 0)
 		notify(player, tprintf("Type: Laser          Regen Time: %s",
 			pWeapon->GetAttributeValue("REGEN TIME")));
 	if(pWeapon->m_type == 1)
 		notify(player, tprintf("Type: Missile        Reload Time: %s",
	...
	if(pWeapon->m_type == 0)
	{
		...
		notify(player, tbuf);
		notify(player, tprintf("Nohull: %d", pLaser->m_nohull));
	}
	if(pWeapon->m_type == 1)
	{
...
HSPACE_COMMAND_HDR(hscAddMessage)
{
	...
	if(Success)
		notify(player, tprintf("%s - Message type %d added.", Name(dbObj), atoi(arg_right)));
	else
		notify(player, tprintf("%s - Failed to add message type.", Name(dbObj)));
}
...
HSPACE_COMMAND_HDR(hscDelMessage)
{
	...
	if(Success)
		notify(player, tprintf("%s - Message type %d deleted.", Name(dbObj), atoi(arg_right)));
	else
		notify(player, tprintf("%s - Failed to delete message type.", Name(dbObj)));
}