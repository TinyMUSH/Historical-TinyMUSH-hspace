...
BOOL CHSPlanet::AddLandingLoc(dbref room)
{
	...
	if (!cLoc)
	{
		sprintf(tbuf, "WARNING: Maximum landing locations reached for planet %s.",
			m_name);
		hs_log(tbuf);
		return FALSE;
	}
...
void CHSPlanet::GiveScanReport(CHSObject *cScanner, 
							   dbref player,
							   BOOL id)
{
	char tbuf[256];
...
void CHSLandingLoc::WriteObjectAttrs(void)
{
	...
	hsInterface.AtrAdd(objnum, "HSDB_ACTIVE", tprintf("%s",
		bActive ? "1" : "0"), GOD);
	hsInterface.AtrAdd(objnum, "HSDB_MAX_CAPACITY",
		tprintf("%d", m_max_capacity), GOD);
	hsInterface.AtrAdd(objnum, "HSDB_CAPACITY",
		tprintf("%d", m_capacity), GOD);
}
