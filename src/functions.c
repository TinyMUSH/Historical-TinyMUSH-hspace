...
/* We add a table() function here which is similar to PennMUSH's table() so
 * that we do not have to rewrite HSpace softcode. We discard the output
 * separator. Other Penn functions are aliased in alias.conf.
 */

FUNCTION(fun_table)
{
	unsigned int spaces, number, ansinumber, striplen, linelen;
	unsigned int count, i;
	int isansi = 0, rturn = 1, cr = 0;
	char *p, *q, *buf, *curr, *objstring, *bp, *cp, sep, osep, *str;

	evarargs_preamble("TABLE", 2, 5);
		
	number = (unsigned int) safe_atoi(fargs[1]);
	linelen = (unsigned int) safe_atoi(fargs[2]);
...
FUNCTION(hs_clone)
{
        safe_str(hsFunction("CLONE", player, fargs), buff, bufc);
}

FUNCTION(hs_set_missile)
{
        safe_str(hsFunction("SETMISSILE", player, fargs), buff, bufc);
}

FUNCTION(hs_sysset)
{
        safe_str(hsFunction("SYSSET", player, fargs), buff, bufc);
}

FUNCTION(hs_addsys)
{
        safe_str(hsFunction("ADDSYS", player, fargs), buff, bufc);
}

FUNCTION(hs_delsys)
{
        safe_str(hsFunction("DELSYS", player, fargs), buff, bufc);
}

FUNCTION(hs_sys_attr)
{
        safe_str(hsFunction("SYSATTR", player, fargs), buff, bufc);
}

FUNCTION(hs_get_missile)
{
        safe_str(hsFunction("GETMISSILE", player, fargs), buff, bufc);
}

FUNCTION(hs_weapon_attr)
{
        safe_str(hsFunction("WEAPONATTR", player, fargs), buff, bufc);
}

FUNCTION(hs_add_weapon)
{
        safe_str(hsFunction("ADDWEAPON", player, fargs), buff, bufc);
}
FUNCTION(hs_get_attr)
{
        safe_str(hsFunction("GETATTR", player, fargs), buff, bufc);
}
FUNCTION(hs_xyang)
{
        safe_str(hsFunction("XYANG", player, fargs), buff, bufc);
}

FUNCTION(hs_zang)
{
        safe_str(hsFunction("ZANG", player, fargs), buff, bufc);
}

FUNCTION(hs_space_msg)
{
        safe_str(hsFunction("SPACEMSG", player, fargs), buff, bufc);
}

FUNCTION(hs_eng_systems)
{
        safe_str(hsFunction("GETENGSYSTEMS", player, fargs), buff, bufc);
}

FUNCTION(hs_set_attr)
{
        safe_str(hsFunction("SETATTR", player, fargs), buff, bufc);
}

FUNCTION(hs_srep)
{
        safe_str(hsFunction("SENSORCONTACTS", player, fargs), buff, bufc);
}

FUNCTION(hs_comm_msg)
{
        safe_str(hsFunction("COMMMSG", player, fargs), buff, bufc);
}
